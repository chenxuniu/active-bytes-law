...
if [[ -n "$(git -C "$repo_root" status --short)" ]]; then
  echo "Qwen2.5-14B holdout requires a clean repository checkout" >&2
...
batch_dir="$results_root/qwen14b-holdout-batch-runs/orders-${start_order}-${end_order}/batch-$batch_tag"
mkdir -p "$batch_dir"
...
report = {
    "schema_version": 1,
    "measurement": "gh200-qwen2p5-14b-holdout-batch-execution-summary",
