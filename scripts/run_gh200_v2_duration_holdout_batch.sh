...
if [[ -n "$(git -C "$repo_root" status --short)" ]]; then
  echo "V2 batch execution requires a clean repository checkout" >&2
...
# Authenticate once while the tmux pane is still attached, then refresh the
# same-terminal sudo ticket so later container launches cannot block overnight.
sudo -v || exit $?
(
  while true; do
    sleep 60
