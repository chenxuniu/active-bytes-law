...
for value in "$start_order" "$end_order"; do
  if [[ ! "$value" =~ ^[0-9]+$ ]] || (( value < 0 || value >= 30 )); then
    echo "orders must be integers from 0 through 29" >&2
    exit 64
  fi
done
...
attempt_runner=${TEL_EVALUATION_ATTEMPT_RUNNER:-$repo_root/scripts/run_gh200_primary_evaluation_attempt.sh}
...
if [[ -n "$(git -C "$repo_root" status --short)" ]]; then
  echo "evaluation batch execution requires a clean repository checkout" >&2
...
batch_dir="$results_root/primary-evaluation-batch-runs/orders-${start_order}-${end_order}/batch-$batch_tag"
mkdir -p "$batch_dir"
...
report = {
    "schema_version": 1,
    "measurement": "gh200-primary-evaluation-batch-execution-summary",
